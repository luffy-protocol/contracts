...
task("deploy-crosschain-novrf", "Deploys the LuffyCrosschainNoVRF contract")
  .addOptionalParam(
    "verify",
  ...
  .setAction(async (taskArgs) => {
    console.log(`Deploying LuffyCrosschainNoVRF contract to ${network.name}`);

    console.log("\n__Compiling Contracts__");
    await run("compile");

    const crosschainContractFactory = await ethers.getContractFactory(
      "LuffyCrosschainNoVRF"
    );
    ...
    console.log(
      "\nDeployed LuffyCrosschainNoVRF contract to:",
    ...
    console.log(
      `\n LuffyCrosschainNoVRF contract deployed to ${crosschainContract.address} on ${network.name}`
    );
  });
